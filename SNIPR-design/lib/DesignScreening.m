...
writeCSVFilefromStructure(sprintf('output/designs/%s_design.csv',name),top_design);

%create a fasta file with the estimated docking sequence (first 33 sequences) of the designed SNIPR
fastafile = sprintf('output/fasta/%s_design.fasta',name);
for index = 2:size(top_design, 1)
    fheader = strcat(name, '_', 'Design ', num2str(top_design{index, 1}));
    fsequence = top_design{index, 2}(1:33); %first 3 nucleotides (GGG) + 30 nucleotides
    fastawrite(fastafile, fheader, fsequence);
end 
