...
$rid=$1;
print "NCBI Request Id: ${rid} \n";

# parse out the estimated time to completion
$response->content =~ /^    RTOE = (.*$)/m;
$rtoe=$1;
print "Estimated time to completion: ${rtoe} minutes\n";
...
@split_fasta = split(/\./, $fastafile);
$design = @split_fasta[0];
$filename =  "$design-BLAST.txt";
print "Blast results filename: $filename\n";
